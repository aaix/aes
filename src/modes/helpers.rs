pub struct PartialBlockHelper<'encoder, 'data> {
    partial_block: &'encoder mut [u8; 16],
    partial_len: &'encoder mut usize,
    data: &'data [u8],
    
    combined_chunk: [u8; 16],
}

impl<'encoder, 'data> PartialBlockHelper<'encoder, 'data> {
    ...
    pub fn new(
        ...
        Self { partial_block, partial_len, data, combined_chunk: [0u8; 16] }
    }


    pub fn take(&mut self) -> Option<&[u8; 16]> {
        ...
        if self.data.len() + *self.partial_len < 16 {
        ...
        if *self.partial_len > 0 {
            ...
            self.combined_chunk[*self.partial_len..16].copy_from_slice(&self.data[0..to_take]);
        ...
        if self.data.len() >= 16 {
            ...
            let chunk = &self.data[0..16];
            self.data = &self.data[16..self.data.len()];
